...
def _render_job_row(job: Job, db, filestore, redis) -> None:
...
def _render_batch_header(batch_id: str, db, filestore, redis) -> None:
    ...
    with col2:
        if failed > 0:
            with st.popover(JOBS_BATCH_RETRY_ALL, key=f"batch_retry_{batch_id}"):
                st.markdown(JOBS_BATCH_RETRY_ALL_CONFIRM)
                if st.button(
                    JOBS_BATCH_RETRY_ALL_CONFIRM_BUTTON,
        ...
        if all_done:
            with st.popover(JOBS_BATCH_DELETE_ALL, key=f"batch_del_{batch_id}"):
                ...
                if st.button(
                    ...
                    st.toast(JOBS_BATCH_DELETE_ALL_SUCCESS.format(count=total))
                    st.rerun()
